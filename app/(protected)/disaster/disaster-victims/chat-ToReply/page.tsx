...
export default function Admin() {
  ...
  const fetchUsersToRely = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/api/disaster/disaster-victims/chat-ToReply?UserId=${UserId}`);
              ...
              <div className="h-5/6 pl-9 flex flex-col gap-2 overflow-y-auto custom-scrollbar">
                {filteredUsers.map((user) => (
                  ...
                  <a
                    key={user.id}
                    className="w-1/3"
                    href={`/disaster/disaster-victims/chat-ToReply?UserId=${user.id}`}
                  >
            ...
            <div className="h-full w-1/4 bg-white rounded-2xl shadow-md shadow-gray-400">
              ...
              <div className="h-3/5 p-5 pt-0">
                <div className="bg-slate-100 h-full rounded-2xl">
                  <p className="text-center font-bold pt-5">Disaster Information</p>
                </div>
              </div>
            </div>
